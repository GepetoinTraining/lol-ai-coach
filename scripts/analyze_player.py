...
from coach.intents import CoachingIntent, PlayerIntent, prompt_for_intent, INTENT_DESCRIPTIONS
...
def parse_riot_id(riot_id: str) -> tuple[str, str]:
    """Parse 'GameName#TAG' into components"""
    if "#" not in riot_id:
        raise ValueError("Invalid format. Use 'GameName#TAG'")
...
async def analyze_player(
    ...
    try:
        with Progress(
            ...
            console.print(f"\n[green]✓[/green] Found: {player['account']['gameName']}#{player['account']['tagLine']}")
            ...
            console.print(f"[green]✓[/green] Retrieved {len(matches)} matches")
            ...
            for match in matches:
                try:
                    ...
                    console.print(f"[yellow]⚠[/yellow] Skipped match: {e}")
            progress.update(task, completed=True)
            
            console.print(f"[green]✓[/green] Processed {len(summaries)} matches")
...
Platforms:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of matches to analyze (default: 20)"
    )
